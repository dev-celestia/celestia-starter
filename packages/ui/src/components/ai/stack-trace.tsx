  ...
  ({ className, children, ...props }: StackTraceHeaderProps) => {
    ...
    return (
      <Collapsible onOpenChange={setIsOpen} open={isOpen}>
        <CollapsibleTrigger className={cn("flex w-full cursor-pointer items-center gap-3 p-3 text-left transition-colors hover:bg-muted/50", className)} {...props}>{children}</CollapsibleTrigger>
      </Collapsible>
    );
  }
);
