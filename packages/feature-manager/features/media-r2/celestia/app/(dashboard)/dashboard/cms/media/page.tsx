...
import { CircleNotch, UploadSimple, Copy, CloudArrowUp } from "@phosphor-icons/react";
import {
  Badge,
  Button,
  Card,
...
import { StorageSettingsDialog } from "@/components/cms/storage-settings-dialog";
...
/** Media library (PRD 3.1): local disk or BYOK Cloudflare R2, WebP variants, alt text. */
export default function MediaPage() {
  const { data: session } = useSession();
  const isAdmin = session?.user.role === "admin";
  const canDelete = session?.user.role === "admin" || session?.user.role === "editor";

  const [assets, setAssets] = useState<(MediaAsset & { url?: string })[] | null>(null);
    ...
    <div className="grid gap-6">
      ...
      {assets === null ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {assets.map((asset) => {
            const url = asset.url ?? `/uploads/${asset.filename}`;
            return (
              <Card key={asset.id} className="overflow-hidden">
                ...
                <CardContent className="space-y-2 p-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    {canDelete && <MediaDeleteButton id={asset.id} onDeleted={load} />}
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    {asset.storage === "r2" && (
                      <Badge variant="secondary" className="gap-1">
                        <CloudArrowUp className="size-3" /> R2
                      </Badge>
                    )}
                  </div>
                </CardContent>
